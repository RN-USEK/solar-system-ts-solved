...
function isMassData(data: any): data is MassData {
  return typeof data === 'object' &&
    typeof data.massValue === 'number' &&
    typeof data.massExponent === 'number';
}

function isVolData(data: any): data is VolData {
  return typeof data === 'object' &&
    typeof data.volValue === 'number' &&
    typeof data.volExponent === 'number';
}

function isPlanet(data: any): data is Planet {
  return typeof data === 'object' &&
    typeof data.id === 'string' &&
    typeof data.name === 'string' &&
    typeof data.isPlanet === 'boolean' &&
    isMassData(data.mass) &&
    isVolData(data.vol) &&
    typeof data.gravity === 'number' &&
    typeof data.avgTemp === 'number' &&
    typeof data.moonsCount === 'number' &&
    Array.isArray(data.moons) &&
    data.moons.every((moon: any) => typeof moon === 'string');
}

function isAsteroid(data: any): data is Asteroid {
  return typeof data === 'object' &&
    typeof data.name === 'string' &&
    typeof data.discoveryYear === 'number' &&
    typeof data.orbitalPeriod === 'number';
}

function isAllData(data: any): data is AllData {
  if (!data || typeof data !== 'object') {
    return false;
  }

  if (!Array.isArray(data.planets) || !Array.isArray(data.asteroids)) {
    return false;
  }

  if (!data.planets.every(isPlanet) || !data.asteroids.every(isAsteroid)) {
    return false;
  }
...
if (isAllData(data)) {
  console.log('Data passes TypeScript type checking.');
} else {
  console.log('Data does not pass TypeScript type checking.');
}
